...
initUhr = initUhrWithClockTime 0 (ClockTime 0 0 0)
initUhrWithClockTime stops (ClockTime hour minute second) = Uhr (initHourZeiger hour) (initMinZeiger minute) (initSecZeiger second) stops
...
uhrStop uhr@(Uhr _ _ _ curStops) = trace "stop" $ switch sf (\_ -> uhrSync stopedUhr (uhrRun stopedUhr) Nothing)
        where sf = constant stopedUhr &&& stopTrigger
              stopedUhr = uhr { stops = curStops + 1 }

uhrRun :: Uhr -> SF AppInput Uhr
uhrRun uhr =  trace "running" $ switch sf (\(currentUhr, targetClockTime) -> uhrStop currentUhr)
                        where 
                          sf :: SF AppInput (Uhr, Event (Uhr, ClockTime))
                          sf = proc input -> do
                                  currentUhr <- currentUhrSF uhr -< input
...
uhrSync (Uhr sourceHourZ sourceMinuteZ sourceSecondZ stops) targetSF _ = trace "sync" $ switch sf (\_ -> targetSF)
        where sf = proc input -> do
                ...
                let nextUhr = (Uhr hourZeiger minZeiger secZeiger stops)
...
currentUhrSF :: Uhr -> SF AppInput Uhr
currentUhrSF uhr = clockTimeInput >>^ (initUhrWithClockTime $ stops uhr)
...
instance Renderer Uhr where
  render (Uhr hZ mZ sZ stops) = scene_ (
          ...
          ++ [circle_ 250 & pos_ (uhrX, uhrY) & colour_ brownColour,
              ...
              text_ 600 100 (pack ("Stops: "++ (show stops))) & pos_ (0, 750) & colour_ blackColour
             ] 
